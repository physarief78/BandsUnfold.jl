#!/usr/bin/env julia
# si_unfold_ntype_chunked.jl
#
# AUTOMATED PIPELINE (N-TYPE DOPED SUPERCELL + Low Memory):
# 1. GENERATE: 2x2x2 Supercell with 1 Phosphorus atom at the center.
...
# 5. PLOT: Unfolds and plots side-by-side (Orange DOS).
...
function build_ntype_supercell_si()
    # 1. Define Geometry
    a_ang = 5.43
    a_bohr = a_ang * 1.8897259886
    nx, ny, nz = 2, 2, 2
    ...
    # 3. DOPING LOGIC (N-Type / Phosphorus)
    # Target: Center of supercell [0.5, 0.5, 0.5]
    ...
    println("    Supercell Construction (N-Type):")
    println("    - Total Atoms: $(length(positions))")
    println("    - Doping: Replacing Atom $dopant_index at $(positions[dopant_index]) with Phosphorus.")

    # 4. Assign Elements (Si and P)
    # The PseudoFamily will automatically fetch 'P.upf' from the Dojo library
    psp_family = PseudoFamily("dojo.nc.sr.lda.v0_4_1.standard.upf")
    
    atoms = Vector{ElementPsp}(undef, length(positions))
    for i in 1:length(positions)
        if i == dopant_index
            atoms[i] = ElementPsp(:P, psp_family)   # Phosphorus (N-Type Dopant)
        else
            atoms[i] = ElementPsp(:Si, psp_family)  # Silicon Host
        end
    end
...
# ==============================================================================
# 3. N-Type Supercell SCF
# ==============================================================================
supercell_file = "ntype_supercell_scf.jld2"
...
if isfile(supercell_file)
    println("\n[1/4] Found cached N-Type Supercell SCF. Loading...")
    dat = JLD2.load(supercell_file)
    l, a, p = build_ntype_supercell_si()
    ...
    println("\n[1/4] Running N-Type Supercell SCF (Si63 P1)...")
    l, a, p = build_ntype_supercell_si()
    model = model_DFT(l, a, p; functionals=PBE(), temperature=1e-3)
...
ax_bands = Axis(fig[1, 1], 
    title = "Unfolded Bands: N-Type Si (Si63 P1)",
    ylabel = "Energy (eV - Ef)",
    xlabel = "Wavevector",
...
# -- Panel 2: DOS --
ax_dos = Axis(fig[1, 2], 
    title = "Total DOS (N-Type)",
    xlabel = "DOS (states/eV)",
    ylabel = "Energy (eV - Ef)", 
)

# Using Orange color for Phosphorus dopant visualization
lines!(ax_dos, dos_values, dos_energies_eV, color=:darkorange, linewidth=2, label="Doped (P)")
...
save("Si_NType_Unfolded.png", fig)
println("Done. Saved to 'Si_NType_Unfolded.png'.")
