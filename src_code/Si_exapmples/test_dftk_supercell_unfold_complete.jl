...
#
# AUTOMATED PIPELINE (Low Memory / 20GB Limit):
# 1. SCF: Computes Density (Drops heavy wavefunctions immediately).
# 2. DOS: Computes Eigenvalues iteratively (Chunked) to avoid storing all ψ.
# 3. BANDS: Computes Path iteratively (Chunked).
# 4. PLOT: Unfolds and plots side-by-side.
...
if n_threads == 1
    @warn "Run with 'julia -t N' (e.g., -t 4) for parallelism."
end
...
const TOL_SCF    = 1e-5
const CHUNK_SIZE = 4     # Reduced chunk size (process 4 k-points at a time) for safety
...
# ==============================================================================
# 3. Supercell SCF (Get Density ONLY)
# ==============================================================================
supercell_file = "supercell_scf.jld2"
...
if isfile(supercell_file)
    println("\n[1/4] Found cached Supercell SCF. Loading...")
    dat = JLD2.load(supercell_file)
    l, a, p = build_supercell_si()
    ...
    println("\n[1/4] Running Supercell SCF (64 Atoms)...")
    l, a, p = build_supercell_si()
    model = model_DFT(l, a, p; functionals=PBE(), temperature=1e-3)
    ...
    # MEMORY OPTIMIZATION: 
    # Immediately save and then DELETE the heavy scfres object (which holds wavefunctions)
    JLD2.save(supercell_file, "rho", ρ_sc, "epsilonF", εF_sc)
    println("Supercell SCF done. Dropping heavy wavefunctions from memory...")
    scfres = nothing 
    GC.gc() # Force cleanup
end
...
# We need eigenvalues for all k-points in the SCF basis.
# Instead of compute_bands(basis) which does all at once, we loop manually.

n_kpoints_scf = length(basis_sc_scf.kpoints)
eigenvalues_sc = Vector{Vector{Float64}}(undef, n_kpoints_scf)

println("    Total SCF k-points: $n_kpoints_scf")
println("    Computing eigenvalues iteratively to save RAM...")

# We iterate 1-by-1 or in small chunks
for (ik, kpt) in enumerate(basis_sc_scf.kpoints)
    # 1. Define a temporary basis for just ONE k-point
    # We use ExplicitKpoints to restrict computation
    # Note: We must pass the k-point in the format expected by ExplicitKpoints
    
    # Get the coordinate for this specific k-point
    k_coord = kpt.coordinate 
    
    # Create an ExplicitKpoints object for just this one point
    kgrid_obj = DFTK.ExplicitKpoints([k_coord])
    
    # 2. Compute bands for this single point
    # ρ=ρ_sc ensures we use the converged density
    res = compute_bands(basis_sc_scf, kgrid_obj; n_bands=320, ρ=ρ_sc)
    
    # 3. Extract Eigenvalues (res.eigenvalues is a Vector of Vectors)
    ...
    if ik % 1 == 0
        @printf "    Processed SCF k-point %d / %d\r" ik n_kpoints_scf
    end
end
...
# Compute DOS summing over contributions
...
println("    Processing in chunks of $CHUNK_SIZE...")
...
for chunk_start in 1:CHUNK_SIZE:total_k
    ...
    # Explicitly clear variables holding wavefunctions for this chunk
    band_data = nothing
    basis_bands = nothing
    
    # Force Garbage Collection to release RAM back to OS
    GC.gc()
end
...
ax_bands = Axis(fig[1, 1], 
    title = "Unfolded Bands: Si 2x2x2 SC",
    ylabel = "Energy (eV - Ef)",
    xlabel = "Wavevector",
...
lines!(ax_dos, dos_values, dos_energies_eV, color=:blue, linewidth=2)
...
Colorbar(fig[1, 3], sc, label="Spectral Weight")
colsize!(fig.layout, 1, Relative(2/3))
colsize!(fig.layout, 2, Relative(1/3))

save("Si_Unfolded_Chunked_DOS.png", fig)
println("Done. Saved to 'Si_Unfolded_Chunked_DOS.png'.")
